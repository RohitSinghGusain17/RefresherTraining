...
public class Program
{
    public static decimal ComputeTotalPayroll(string[] employees)
    {
        ...
        foreach (var emp in employees)
        {
            ...
            {
                case "H":
                    list.Add(new HourlyEmployee(decimal.Parse(parts[1]), decimal.Parse(parts[2])));
                    break;

                case "S":
                    list.Add(new SalariedEmployee(decimal.Parse(parts[1])));
                    break;

                case "C":
                    list.Add(new CommissionEmployee(decimal.Parse(parts[1]), decimal.Parse(parts[2])
                    ));
                    break;
            }
        }
